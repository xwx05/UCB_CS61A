...
def buy(fruits_to_buy, prices, total_amount):
    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:  # 说明已经考虑完所有水果且刚好花完预算
            print(cart)
        elif fruits and amount > 0:
            fruit = fruits[0]  # 先取出第一种水果
            price = prices[fruit]
            for k in range(1, amount // price + 1):  # 计算在当前预算下该水果最多能购买的数量
                # Hint: The display function will help you add fruit to the cart.
                add(fruits[1:], amount - k * price, cart + display(fruit, k))  # 对每个数量可能进行排列组合，并且要确保每种水果都至少买了一个
