...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        """
        self.name = name
        self.exit = exit  # place左侧是exit，右侧是entrance
        self.bees = []        # A list of Bees  一个place上可能有多只蜜蜂
...
class Ant(Insect):
    ...
    food_cost = 0  # 放置每种ant有不同的食物消耗
    is_container = False
    blocks_path = True

    def __init__(self, health=1):
        super().__init__(health)
        self.doubled = False  # 使用instance attribute跟踪ant的伤害是否double
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if self.can_contain(place.ant):  # self是本次新添加的蚂蚁，place.ant是该位置上已有的
                self.store_ant(place.ant)
                place.ant = self  # place.ant总是指的 container_ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        distance = 0
        while current_place and not current_place.is_hive:  # 一直找到hive的位置，都没有bee，将返回None
            if self.lower_bound <= distance <= self.upper_bound:
                bee = random_bee(current_place.bees)  # 从当前place的bee列表中随机选择一个
                if bee:
                    return bee
            current_place = current_place.entrance  # 如果当前place没有bee，就向右移动一个place
            distance += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:  # 先判断当前fire ant是否会死亡
            reflective_damage += self.damage
        for bee in self.place.bees[:]:  # 遍历当前place上的所有bee，减少bee的health
            bee.reduce_health(reflective_damage)   # 当health减至0后，就会执行remove_from (在Insect类中实现的)
        super().reduce_health(amount)  # 不能先执行这句，否则ant已被从当前place移除，self.place is None，就不能对bee执行操作了
        """
        如果使用self.reduce_health 会调用当前类的 reduce_health 方法。
        在 FireAnt 的 reduce_health 方法中调用 self.reduce_health，会再次进入同一个方法。
        这个过程会不断重复，直到 Python 抛出 RecursionError（递归深度超出限制）
        super()能自动找到下一个实现了reduce_health方法的父类
        """
        # END Problem 5
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        if self.place is not None:
            bees = self.place.bees[:]  # 复制蜜蜂列表，避免在迭代时修改
            for bee in bees:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem 12
        super().action(gamestate)  # 执行标准的ThrowerAnt动作
        ...
        while current_place:
            ...
            if current_ant:
                ...
                if current_ant.is_container and current_ant.ant_contained:
                    current_ant.ant_contained.double()  # 如果container ant包含其他ant，对被contained的ant也要double
    ...
    def throw_at(self, target):
        ...
        if target:  # 确保目标存在
            ...
            if hasattr(target, 'slow_turns'):
                target.slow_turns = 5  # 可以动态地为 target（即 Bee 实例）添加 slow_turns 属性
            else:
                # 否则，初始化减速回合数并保存原始的 action 方法
                target.slow_turns = 5
                target.original_action = target.action

            # 定义减速时的 action 覆盖原 action
            def slowed_action(gamestate):
                if target.slow_turns > 0:  # 处于减速状态
                    if gamestate.time % 2 == 0:  # 为奇数时不采取动作
                        target.original_action(gamestate)
                    target.slow_turns -= 1
                else:
                    # 减速效果结束，恢复原始的 action 方法
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 2
        if target:  # 确保目标存在
            target.scare(2)  # 调用蜜蜂的 scare 方法，设置后退回合数为 2
            original_action = target.action  # 暂存原始的 action 方法

            def scared_action(gamestate):
                if hasattr(target, 'scared') and target.scared and target.scare_turns > 0:
                    # 如果蜜蜂被 SlowThrower 减速且当前回合为奇数，则不后退
                    if hasattr(target, 'slow_turns') and gamestate.time % 2 != 0:
                        pass
                    else:
                        # 如果蜜蜂不在蜂巢旁边，则后退
                        if not target.place.is_hive and not target.place.entrance.is_hive:
                            if target.place.ant is not None:
                                target.place.ant.reduce_health(self.damage)
                            target.move_to(target.place.entrance)  # 后退到上一个位置
                        target.scare_turns -= 1  # 减少剩余后退回合数
                        if target.scare_turns == 0:
                            target.scared = False  # 后退效果结束
                            target.action = original_action
                else:
                    # 正常行为
                    original_action(gamestate)

            target.action = scared_action
        # END Problem EC 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        # 检查 LaserAnt 所在位置的昆虫
        if current_place.ant is not self:  # 排除自身
        ...
        # 检查前面的位置
        while current_place.entrance is not None and not current_place.entrance.is_hive:
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem EC 4
        base_damage = self.damage - 0.25 * distance - 0.0625 * self.insects_shot
...
class Bee(Insect):
    ...
    def scare(self, length):
        ...
        """
        # BEGIN Problem EC 2
        if not hasattr(self, 'scared'):  # 如果蜜蜂没有被恐吓过
            self.scared = True
            self.scare_turns = length  # 设置后退回合数
        # END Problem EC 2
