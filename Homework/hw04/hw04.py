def shuffle(s):
    ...
    for i in range(len(s)//2):
        # s_copy[2*i] = left[i]  # 报错：不能通过索引直接向空列表中添加元素
        # s_copy[2*i+1] = right[i]
        s_copy.append(left[i])
        s_copy.append(right[i])
    return s_copy
...
## Exam Practice
# Summer 2021 MT2 Q4: Maximum Exponen-tree-ation
def exp_tree(values):
    """
...
# Summer 2019 MT1 Q8: Leaf It To Me
def max_path(t, k):
    ...
    def helper(t, k, on_path):
        if k == 0:  # 说明路径长度已达上限
            ...
            return [label(t)]
        a = [[label(t)] + helper(b, k - 1, True) for b in branches(t)]  # 得到了从当前节点label(t)开始的所有可能路径列表
        if on_path:
            return max(a, key = sum)
        else:
            b = [helper(b, k, False) for b in branches(t)]  # 探索不含label(t)的路径列表
        return max(a + b, key = sum)  # 将列表 a 和 b 合并，然后从中选择路径节点标签和最大的路径并返回。
